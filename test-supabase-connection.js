...
async function testConnection() {
    console.log('[TOOL] Supabase 연결 테스트 시작...\n');
    ...
    try {
        ...
        // 1. movies 테이블 존재 확인
        console.log('[INFO] movies 테이블 조회 중...');
        ...
        if (moviesError) {
            console.error('[ERROR] movies 테이블 조회 실패:', moviesError.message);
        } else {
            console.log('[SUCCESS] movies 테이블 연결 성공');
        }
        ...
        if (insertError) {
            console.error('[ERROR] 데이터 삽입 실패:', insertError.message);
            console.error('상세 오류:', insertError);
        } else {
            console.log('[SUCCESS] 테스트 데이터 삽입 성공');
            ...
            if (deleteError) {
                console.error('[ERROR] 데이터 삭제 실패:', deleteError.message);
            } else {
                console.log('[SUCCESS] 테스트 데이터 삭제 성공');
            }
        }
