...
async function testKobisIntegration() {
    console.log('[MOVIE] KOBIS API 통합 테스트 시작\n');

    // KOBIS API 키 확인
    if (!process.env.KOBIS_API_KEY) {
        console.log('[WARN]  KOBIS_API_KEY가 설정되지 않았습니다.');
        console.log('[MEMO] .env 파일에 KOBIS_API_KEY=your_api_key_here 를 추가해주세요.\n');
        console.log('[LINK] KOBIS API 키 발급: http://kobis.or.kr/kobisopenapi/homepg/main/main.do');
        return;
    }
    ...
    try {
        // 1. 일별 박스오피스 테스트
        console.log('[INFO] 일별 박스오피스 조회 테스트...');
        const boxOffice = await kobis.getDailyBoxOffice();
        
        if (boxOffice.success && boxOffice.data.dailyBoxOfficeList) {
            console.log('[SUCCESS] 박스오피스 조회 성공');
            console.log(`[TOMORROW] 기준일: ${boxOffice.data.boxofficeType} ${boxOffice.data.showRange}`);
        ...
        if (boxOffice.success && boxOffice.data.dailyBoxOfficeList.length > 0) {
            ...
            if (searchResult.success && searchResult.data.movieList.length > 0) {
                ...
                if (movieDetail.success) {
                    ...
                    console.log('[ERROR] 상세정보 조회 실패\n');
                }
            } else {
                console.log('[ERROR] 영화 검색 실패\n');
            }
        }

        // 4. 통합 DataExtractor 테스트 (간단 버전)
        console.log('[LINK] DataExtractor 통합 테스트...');
        const naverConfig = {
        ...
        console.log('[SUCCESS] DataExtractor with KOBIS 초기화 성공');
        console.log('[TIP] 실제 영화 검색 테스트는 카카오 스킬 서버 실행 후 테스트하세요.\n');

        // 5. F1 더무비 테스트 시뮬레이션
        console.log('[RACECAR]  F1 더무비 검색 시뮬레이션...');
        const f1Search = await kobis.searchMovies('F1');
        if (f1Search.success && f1Search.data.movieList.length > 0) {
            console.log('[SUCCESS] F1 관련 영화 검색 결과:');
            f1Search.data.movieList.slice(0, 3).forEach(movie => {
                console.log(`   - ${movie.movieNm} (${movie.openDt})`);
            });
        } else {
            console.log('[TIP] F1 관련 영화는 현재 KOBIS에서 찾을 수 없습니다.');
            console.log('   실제 테스트는 존재하는 영화명으로 해보세요.');
        }

        console.log('\n[PARTY] KOBIS API 통합 테스트 완료!');
