...
async function getWeatherInfo(cityKorean = '서울') {
    try {
        console.log(`[WEATHER] 네이버 날씨 정보 조회: ${cityKorean}`);
        ...
        console.log('[WARN] 날씨 크롤링 실패, 뉴스 검색으로 전환');
        ...
        if (weatherNews && weatherNews.length > 0) {
            ...
            let weatherInfo = `[WEATHER] ${cityKorean} 날씨 정보\n\n[NEWS] 최신 날씨 뉴스:\n`;
        ...
        console.error('[ERROR] 날씨 정보 검색 오류:', error.message);
        return getWeatherFallback();
    }
}
...
function getWeatherGeneralInfo(city) {
    const koreanTime = getKoreanDateTime();
    return `[WEATHER] ${city} 날씨 정보\n\n[TIP] 실시간 날씨 확인 방법:\n• 네이버 날씨: weather.naver.com\n• 기상청: weather.go.kr\n• 휴대폰 날씨 앱\n• "날씨" 검색\n\n[APP] 추천 앱:\n• 날씨 (기본 앱)\n• 미세미세 (미세먼지)\n• WeatherBug\n\n[TIME] 안내 시간: ${koreanTime.formatted}\n\n[TEMP] 정확한 기온, 습도, 바람 정보는 위 사이트를 확인해주세요.`;
}
...
function getWeatherIcon(mainWeather) {
    ...
    return icons[mainWeather] || '[WEATHER]';
}

// 날씨 API 실패시 폴백 함수
function getWeatherFallback() {
    const koreanTime = getKoreanDateTime();
    return `[WEATHER] 날씨 정보 서비스\n\n[WARN] 현재 날씨 API에 연결할 수 없습니다.\n\n[TIP] 대안:\n• 네이버 날씨: weather.naver.com\n• 기상청: weather.go.kr\n• 휴대폰 날씨 앱 확인\n\n[TIME] 확인 시간: ${koreanTime.formatted}\n\n[APP] 정확한 날씨는 기상청이나 날씨 앱을 확인해주세요.`;
}
...
function isMovieReviewRequest(message) {
    ...
    console.log('[MOVIE] 모든 영화평 요청은 새로운 시스템에서 처리');
    return false;
}
...
async function generateNaturalResponse(message) {
    try {
        console.log(`[MSG] 자연스러운 대화 응답 생성: "${message}"`);
        
        // 감정 분석
        const emotion = analyzeEmotion(message);
        console.log(`[SMILE] 감지된 감정: ${emotion}`);
        ...
        console.error('[ERROR] 자연스러운 응답 생성 오류:', error);
        return '[SMILE] 무엇을 도와드릴까요? 더 구체적으로 말씀해주시면 좋은 조언을 드릴 수 있어요!';
    }
}
...
function generateHeatStruggleResponse(message) {
    const responses = [
        `[TEMP] 이 더위에 정말 힘드시겠어요!\n\n[TIP] 더위 극복 팁:\n• 실내 운동 (홈트레이닝, 계단 올라가기)\n• 새벽/저녁 시간대 활용 (6-7시, 8-9시)\n• 에어컨 있는 헬스장이나 수영장\n• 충분한 수분 섭취 필수!\n\n[SMILE] 무리하지 마시고 건강이 최우선이에요!`,
        
        `[SUN] 요즘 날씨가 정말 살인적이네요!\n\n[HOUSE] 실내 대안:\n• 요가, 필라테스, 스트레칭\n• 계단 오르내리기 (아파트 계단 활용)\n• 온라인 홈트레이닝 영상\n• 쇼핑몰이나 지하철역 걷기\n\n[STRONG] 조금씩이라도 꾸준히 하는 게 중요해요!`,
        
        `[SWEAT] 이런 더위에 운동 생각하시다니 대단해요!\n\n[CLOCK] 시간대별 추천:\n• 새벽 5-6시: 공원 산책, 조깅\n• 밤 9-10시: 야간 운동, 헬스장\n• 실내: 언제든 홈트레이닝\n\n🚿 운동 후엔 시원한 샤워로 체온 조절하세요!`
    ];
...
function generateFrustrationResponse(message) {
    const responses = [
        `😔 힘든 일이 있으신 것 같네요.\n\n[HUG] 괜찮아요, 다들 그런 때가 있어요!\n• 잠시 쉬어가는 것도 필요해요\n• 좋아하는 음악 들으며 산책\n• 친구나 가족과 대화\n• 충분한 휴식과 수면\n\n[STRONG] 이 또한 지나갈 거예요!`,
        
        `[SMILE] 힘드시는 마음 이해해요.\n\n[RAINBOW] 기분 전환 방법:\n• 맛있는 것 먹기\n• 좋아하는 영화나 드라마\n• 따뜻한 차 한 잔\n• 반신욕이나 족욕\n\n[SPARKLE] 내일은 분명 더 나은 날이 될 거예요!`
    ];
...
function generateGeneralConversationalResponse(message) {
    const responses = [
        `[SMILE] 좋은 대화 같아요! 계속 이야기해봐요!\n\n[MSG] 어떤 이야기를 나누고 싶으신가요?\n• 일상적인 고민이나 걱정\n• 취미나 관심사\n• 오늘 있었던 일\n• 궁금한 정보\n\n[SPARKLE] 편하게 말씀해주세요!`,
        
        `[HUG] 무엇을 도와드릴까요?\n\n[TARGET] 이런 것들을 물어보실 수 있어요:\n• 일상 고민 상담\n• 정보 검색 (뉴스, 맛집, 쇼핑)\n• 영화 추천이나 평점\n• 건강이나 운동 조언\n\n💫 자유롭게 이야기해주세요!`,
    ...
    try {
        ...
        if (!movieResults || movieResults.length === 0) {
            ...
            if (newsResults && newsResults.length > 0) {
                let newsInfo = `[MOVIE] "${movieTitle}" 영화평 뉴스 검색 결과\n\n`;
                newsInfo += `[NEWS] 관련 뉴스/리뷰:\n`;
        ...
        console.log(`[DRAMA] Playwright 크롤링 시도: "${bestMatch.title}"`);
        ...
        try {
            ...
            if (crawlResults && crawlResults.naver) {
                console.log('[SUCCESS] Playwright 크롤링 성공 - 상세 정보 제공');
                const result = crawler.formatForKakaoSkill(crawlResults, bestMatch.title);
                await crawler.close();
                return result.data.message;
            } else {
                console.log('[WARN] Playwright 크롤링 실패 - API 정보로 폴백');
                await crawler.close();
            }
        } catch (crawlerError) {
            console.log(`[WARN] Playwright 크롤링 오류: ${crawlerError.message}`);
        }
        
        // 3단계: API 정보로 기본 영화평 생성 (폴백)
        let movieReviewText = `[MOVIE] "${bestMatch.title}" 영화평\n\n`;
        ...
        if (bestMatch.userRating && bestMatch.userRating !== '0.00') {
            ...
            movieReviewText += `\n[FAVORITE] 네이버 평점: ${rating}/10 ${stars}\n`;
...
app.get('/', (req, res) => {
    ...
    res.send(`
        ...
        <p><strong>날씨 정보 (네이버 뉴스):</strong> ${(hasNaverClientId && hasNaverClientSecret) ? '[SUCCESS] 사용 가능' : '[ERROR] 네이버 API 필요'}</p>
        <hr>
        ...
        <ul>
            ...
            <li>[BABY] 출산 신고 안내 (예: "애기 태어나면 신고해야 할 부분")</li>
            <li>[WEATHER] 날씨 정보 제공 (예: "서울 날씨", "부산 날씨")</li>
...
app.post('/api/crawl-movies', async (req, res) => {
    try {
        ...
        console.error('[ERROR] 수동 크롤링 API 오류:', error);
        res.status(500).json({
            success: false,
...
app.get('/api/scheduler-status', (req, res) => {
    try {
        ...
        console.error('[ERROR] 스케줄러 상태 확인 오류:', error);
        res.status(500).json({
            success: false,
            error: error.message
        });
...
app.post('/kakao-skill-webhook', async (req, res) => {
    ...
    try {
        ...
        // Claude AI 버전 문의 - 더 정확한 패턴 매칭
            ...
            responseText = `[AI] 현재 Claude AI 정보\n\n[APP] 모델: Claude 3.5 Sonnet (2024-10-22)\n[LIGHTNING] 성능: 최신 고성능 모델\n[BRAIN] 특징: 향상된 추론 능력과 빠른 응답 속도\n\n[TIME] 현재 시간: ${koreanTime.formatted}\n[TOMORROW] 오늘 날짜: ${koreanTime.date}`;
        }
        // 즉시 응답 가능한 간단한 질문들
        ...
        */
        ...
        console.log(`[OUTBOX] 최종 응답 길이: ${responseText ? responseText.length : 0}자`);
        ...
        console.error('[ERROR] 전체 요청 처리 중 오류:', error);
        ...
        const errorResponse = {
            ...
            template: {
                outputs: [{
                    simpleText: {
                        text: '[WARN] 일시적인 오류가 발생했습니다. 잠시 후 다시 시도해주세요.'
                    }
                }]
            }
        };
...
app.listen(PORT, () => {
    ...
    console.log(`[KEY] Claude AI API 키 상태: ${process.env.CLAUDE_API_KEY ? '설정됨 (' + process.env.CLAUDE_API_KEY.length + '자)' : '미설정'}`);
    console.log(`[SATELLITE] 네이버 API 키 상태: ${(process.env.NAVER_CLIENT_ID && process.env.NAVER_CLIENT_SECRET) ? '설정됨' : '미설정'}`);
    console.log(`[WEATHER] 날씨 정보: 네이버 뉴스 검색 활용`);
    console.log(`🗄️ Supabase 상태: ${(process.env.SUPABASE_URL && process.env.SUPABASE_SERVICE_ROLE_KEY) ? '설정됨' : '미설정'}`);
    console.log(`[MOVIE] 영화진흥위원회 API 상태: ${process.env.KOFIC_API_KEY ? '설정됨' : '미설정'}`);
    
    // 영화 데이터 자동 업데이트 스케줄러 시작
    try {
        movieScheduler.start();
        console.log('[TOMORROW] 영화 데이터 자동 업데이트 스케줄러 시작됨');
    } catch (error) {
        console.error('[ERROR] 스케줄러 시작 실패:', error);
    }
});
