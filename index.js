...
app.post('/kakao-skill-webhook', async (req, res) => {
    ...
    try {
        ...
        // 즉시 응답: 화난 표현이나 대화형 AI 요청 처리 (Claude API 호출 없이)
        const angryExpressions = /똑똑|정신차려|정신차렷|박지피티|먹통|다운|안돼|안해|말해|응답|반응|바보|멍청|화나|짜증|답답|대화형|진화/i;
        const conversationalRequests = /대화형|ai|진화|똑똑|채팅|대화/i;
        
        if (angryExpressions.test(userMessage) || conversationalRequests.test(userMessage)) {
            console.log('😅 화난 표현 또는 대화형 요청 감지 - 즉시 응답');
