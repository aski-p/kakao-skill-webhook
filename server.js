...
const ROUTER_VERSION = 'context-aware-fallback-2026-05-17';
...
function isBrainHealthQuestion(message, state = {}) {
  ...
  if (/브레인\s*포그|브레인포그|인지|인지능력|시지각|지각능력|집중력|기억력|뇌|두뇌|주의력|멍함|머리.*안|의도.*알아|디테일|자세히|추천해줘/i.test(message) && state.topic === 'brain_health') return true;
  return /브레인\s*포그|브레인포그|인지|인지능력|시지각|지각능력|집중력|기억력|뇌|두뇌|주의력|멍함|머리.*안/i.test(message);
}

function isPreferenceRecommendation(message, state = {}) {
  if (hasBuyCue(message) || hasPriceCue(message) || isExplicitSearchQuery(message) || isWeatherQuery(message) || isCalendarHolidayQuery(message) || state.topic === 'brain_health') return false;
  if (state.topic === '위스키' && /(추천|비슷한|맛난|맛있는|스타일|끌리|안끌리|다른\s*것|좋아해|취향|골라|어울려|뭐가\s*좋|더|비싼|강한|부드러운|달달한|스모키|피트|셰리|버번|마실)/.test(message)) return true;
  return false;
}
...
function buildFastPreferenceAnswer(message, userId) {
  ...
  return buildWhiskyAnswer(message, state);
}

function buildTimeoutFallback(message, userId) {
  const state = getState(userId);
  if (isBrainHealthQuestion(message, state)) return buildBrainHealthAnswer(message, state);
  return buildFastPreferenceAnswer(message, userId) || '답변이 늦어질 것 같아서 짧게 먼저 말할게. 네 질문은 검색 결과를 던지는 게 아니라, 방금 맥락을 이해해서 설명해야 하는 쪽이야. 조금 더 구체적으로 물어보면 그 부분부터 바로 이어서 말해줄게.';
}
...
function buildHolidayAnswer(message) {
  ...
  const items = [];
  for (let day = 1; day <= lastDay; day += 1) { const key = toDateKey(year, month, day); const dow = new Date(Date.UTC(year, month - 1, day)).getUTCDay(); if (dow === 0 || dow === 6) restDays.add(key); if (holidays[key]) { restDays.add(key); items.push({ day, dow, name: holidays[key] }); } }
  ...
  if (items.length) { lines.push('공휴일은 이렇게 있어:'); items.forEach((item) => lines.push(`- ${item.day}일(${weekdayKo(year, month, item.day)}): ${item.name}`)); }
...
function isRelevantShoppingItem(query, item) {
  ...
  if (/5090/.test(q)) { const accessoryWords = /케이블|cable|라이저|riser|브라켓|bracket|수냉|워터블럭|water\s*block|백플레이트|쿨러|fan|팬|방열판|거치대|스탠드|지지대|홀더|커버|부품|부속|박스|메인보드|파워|케이스/; const gpuWords = /rtx|geforce|지포스|그래픽카드|그래픽 카드|vga|gpu/; if (accessoryWords.test(title) || !gpuWords.test(title) || item.lprice < 2500000) return false; }
...
function buildSystemPrompt(results, route, state, analysis) {
  ...
  return ['너는 카카오톡에서 대화하는 친근한 한국어 AI 친구야.', '모든 답변은 자연스러운 반말로 해. 존댓말, ~요, ~습니다 말투는 쓰지 마.', '사용자 말을 먼저 이해하고, 카톡 대화처럼 짧고 자연스럽게 받아쳐.', '건강/인지 관련 질문은 진단처럼 단정하지 말고 가능성과 확인 방향을 말해. 위험 신호가 있으면 진료를 권해.', '취향 추천은 쇼핑 검색이 아니라 대화 맥락과 취향을 기준으로 해. 구매처/가격/최저가/판매 모델을 묻는 경우에만 쇼핑 맥락으로 말해.', '찾아볼게/기다려줘처럼 미래에 도구를 실행할 척하지 마.', '카카오톡 제한시간이 짧으니까 보통 1~5문장으로 답해.', `현재 한국 시간: ${getKoreanDateTime()}`, `라우터: ${route.intent}/${route.handler}, source=${route.source || 'unknown'}`, `분석: ${JSON.stringify(analysis || {})}`, `대화 상태: ${JSON.stringify(state || {})}`, searchContext ? `검색 결과:\n${searchContext}` : ''].filter(Boolean).join('\n');
}
...
async function buildAnswer(message, userId) {
  ...
  if (isBrainHealthQuestion(message, state)) {
    const route = routeFromIntent('chat', 0.98, 'fast_brain_health');
    const analysis = { intent: 'chat', tool: 'none', topic: 'brain_health', source: 'fast_brain_health' };
    return { answer: buildBrainHealthAnswer(message, state), results: [], route, topic: 'brain_health', analysis };
  }
