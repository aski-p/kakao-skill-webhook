...
const ROUTER_VERSION = 'context-aware-support-router-2026-05-17b';
...
function rememberMessage(userId, role, content, route, topic, analysis) {
  ...
  dialogueState.set(userId, { ...previous, lastIntent: route?.intent || previous.lastIntent, topic: inferTopic(sourceText, topic || previous.topic || ''), slots: { ...previous.slots, lastAnalysis: analysis || previous.slots?.lastAnalysis }, turns: (previous.turns || 0) + (role === 'assistant' ? 1 : 0), updatedAt: Date.now() });
}
...
const isWeatherQuery = (message) => /(날씨|기온|비\s*와|눈\s*와|미세먼지|습도|강수|온도)/.test(message);
...
function isBrainHealthQuestion(message, state = {}) { if (hasBuyCue(message) || hasPriceCue(message) || isWeatherQuery(message) || isCalendarHolidayQuery(message)) return false; if (/브레인\s*포그|브레인포그|인지|인지능력|시지각|지각능력|집중력|기억력|뇌|두뇌|주의력|멍함|머리.*안/i.test(message)) return true; return state.topic === 'brain_health' && /(의도.*알아|질문.*의도|디테일|자세히|추천해줘|어떻게|왜|원인|검사|병원)/.test(message); }
function isEmotionalAdviceQuestion(message, state = {}) { if (isWeatherQuery(message) || isCalendarHolidayQuery(message) || isPriceQuery(message) || isExplicitPurchaseRecommendation(message)) return false; if (/(화가\s*나|화나|짜증|속상|억울|서운|불쾌|불편|어떻게\s*해야|어떡|고민|친구|가족|회사|상사|연인|거래|사갔|빌려|빌려줬|컴퓨터.*더러|더러워)/.test(message)) return true; return state.topic === 'support' && /(어떻게|뭐라고|말해|답장|사과|화나|속상|괜찮|해야)/.test(message); }
function isPreferenceRecommendation(message, state = {}) { if (hasBuyCue(message) || hasPriceCue(message) || isExplicitSearchQuery(message) || isWeatherQuery(message) || isCalendarHolidayQuery(message) || state.topic === 'brain_health' || state.topic === 'support') return false; return state.topic === '위스키' && /(추천|비슷한|맛난|맛있는|스타일|끌리|안끌리|다른\s*것|좋아해|취향|골라|어울려|뭐가\s*좋|더|비싼|강한|부드러운|달달한|스모키|피트|셰리|버번|마실)/.test(message); }
...
function analyzeTurn(message, userId) { const state = getState(userId); if (isContinuationRequest(message)) return { intent: 'continuation', tool: 'none', confidence: 1, topic: state.topic || '', productQuery: '', searchQuery: '', source: 'heuristic' }; if (isCalendarHolidayQuery(message)) return { intent: 'calendar_holiday', tool: 'calendar', confidence: 0.98, topic: '', productQuery: '', searchQuery: '', source: 'heuristic' }; if (isWeatherQuery(message)) return { intent: 'weather', tool: 'weather', confidence: 0.95, topic: '', productQuery: '', searchQuery: '', source: 'heuristic' }; if (isPriceQuery(message)) return { intent: 'price', tool: 'shopping', confidence: 0.92, topic: '', productQuery: getShoppingQuery(message), searchQuery: '', source: 'heuristic' }; if (isExplicitPurchaseRecommendation(message)) return { intent: 'shopping_recommendation', tool: 'shopping', confidence: 0.9, topic: '', productQuery: getProductQueryFromPurchase(message), searchQuery: '', source: 'heuristic_purchase' }; if (isNewsQuery(message)) return { intent: 'news_search', tool: 'news', confidence: 0.82, topic: '', productQuery: '', searchQuery: getSearchQuery(message), source: 'heuristic' }; if (isExplicitSearchQuery(message)) return { intent: 'web_lookup', tool: 'web', confidence: 0.78, topic: '', productQuery: '', searchQuery: getSearchQuery(message), source: 'heuristic' }; return { intent: 'chat', tool: 'none', confidence: 0.72, topic: inferTopic(message, state.topic), productQuery: '', searchQuery: '', source: 'heuristic' }; }

function buildBrainHealthAnswer(message, state = {}) { if (/의도.*알아|질문.*의도|내가.*뭘.*묻/.test(message)) return '응, 네 의도는 “내가 말한 인지 저하, 브레인포그, 시지각 문제가 실제로 뇌 기능이랑 연결될 수 있는지, 그리고 어느 정도로 심각하게 봐야 하는지”를 묻는 거야.\n\n연관은 있을 수 있어. 다만 그게 곧바로 뇌 손상이나 큰 병이라는 뜻은 아니고, 수면 부족, 스트레스, 불안/우울, ADHD 성향, 약물, 혈당/갑상선/빈혈, 눈 문제, 편두통, 전정기관 문제 같은 것들도 비슷하게 만들 수 있어.'; if (/디테일|자세히|구체/.test(message)) return '좀 자세히 말하면, 브레인포그는 “생각이 느림, 집중 유지가 안 됨, 말이나 기억이 바로 안 떠오름” 같은 실행기능/주의력 쪽 문제로 느껴질 수 있어. 시지각은 눈으로 본 정보를 뇌가 해석하는 과정이라서, 단순 시력 말고도 공간감, 글자 읽기, 움직임 추적, 피로감이 같이 엮일 수 있고.\n\n그래서 네 말처럼 인지능력과 시지각이 같이 흔들리면 뇌와 무관하다고 보긴 어려워. 다만 원인은 넓어. 뇌 자체 문제일 수도 있지만 수면, 스트레스, 불안, 우울, ADHD, 눈의 조절/사위 문제, 편두통, 빈혈/갑상선/비타민 결핍도 가능해.'; if (/추천/.test(message)) return '추천은 이렇게 해볼게.\n\n1. 먼저 수면 시간, 카페인, 스트레스, 복용 약, 증상이 심한 시간대를 1~2주 기록해봐.\n2. 시야가 흐리거나 글자가 흔들리면 안과에서 시력 말고 조절/사위/안구운동 쪽도 봐달라고 해.\n3. 브레인포그가 계속되면 혈액검사로 빈혈, 갑상선, 비타민 B12/D, 혈당 쪽을 확인해봐.\n4. 갑자기 악화, 한쪽 힘 빠짐, 말 어눌함, 심한 두통, 시야 일부가 안 보임은 바로 응급 쪽이야.'; return '응, 연관이 있을 수 있어. 인지능력, 브레인포그, 시지각은 전부 “뇌가 정보를 받아들이고 처리하고 집중을 유지하는 과정”이랑 연결돼.\n\n다만 이게 곧바로 심각한 뇌질환이라는 뜻은 아니야. 수면 부족, 스트레스, 불안/우울, ADHD 성향, 눈의 조절 문제, 편두통, 빈혈/갑상선/비타민 문제 같은 것도 비슷하게 만들 수 있어.'; }
function buildEmotionalAdviceAnswer(message, state = {}) { if (/친구.*컴퓨터|컴퓨터.*사갔|사갔.*더러|더러워.*화/.test(message)) return '그건 화날 만해. 일단 바로 감정 섞어서 따지기보단, 먼저 “어느 부분이 더러웠는지” 사진이나 상태를 확인하고 기준을 정하는 게 좋아.\n\n이미 팔기로 한 물건이면 네가 할 수 있는 건 보통 3가지야. 1. 미안하다고 하고 청소비/소액 환불을 제안하기, 2. 다시 가져와서 네가 청소해주기, 3. 판매 전 상태를 서로 오해한 거면 차분히 설명하기.\n\n친구 관계가 중요하면 “기분 나빴겠다. 내가 확인을 덜 했네. 어느 부분이 제일 문제였는지 사진 보내줘. 가능한 선에서 처리할게” 정도로 보내는 게 제일 덜 싸워.'; if (/뭐라고|답장|말해/.test(message)) return '이렇게 보내면 무난해.\n\n“기분 나빴겠다. 내가 상태를 제대로 확인 못 한 것 같아. 어느 부분이 제일 문제였는지 사진으로 보내줘. 내가 청소해주거나, 필요한 만큼 조정할 수 있는지 볼게.”\n\n핵심은 사과는 짧게 하고, 바로 해결 선택지를 주는 거야. 감정싸움으로 가면 손해가 커져.'; return '네 의도는 “지금 감정이 올라왔는데, 관계를 망치지 않으면서 어떻게 대응해야 하냐”는 거야. 우선 바로 따지기보다 상대가 불편했던 지점을 확인하고, 네가 해줄 수 있는 범위를 정해서 말하는 게 좋아. 감정은 인정하되 책임 범위는 차분히 나누는 쪽이 제일 안전해.'; }
function buildCapabilityAnswer(message) { if (/위스키|whisky|whiskey/i.test(message)) return '응, 위스키 추천 가능해. 바로 쇼핑 검색부터 하는 게 아니라 먼저 취향을 잡는 게 맞아.\n\n입문이면 부드럽고 달달한 쪽, 스모키한 향 좋아하면 피트 쪽, 하이볼용이면 가성비 좋은 블렌디드 쪽으로 나눠서 추천해줄 수 있어.'; if (/추천/.test(message)) return '응, 추천 가능해. 바로 검색부터 하지 말고 먼저 용도랑 취향을 잡아야 제대로 골라줄 수 있어.'; return '응, 그건 도와줄 수 있어. 바로 검색이 필요한 건 검색해서 알려주고, 그냥 취향이나 생각 정리가 필요한 건 대화하면서 맞춰줄게.'; }
function buildSmallTalkAnswer(message) { if (/뭐해|뭐\s*하고/.test(message)) return '나 지금 여기서 네 말 보고 있었지. 이제 잡담이랑 검색을 더 확실히 나눠서 받게 고쳤어.'; if (/쉬고\s*싶|피곤|졸려/.test(message)) return '그럴 땐 진짜 잠깐 쉬는 게 맞아. 머리 계속 굴리면 더 꼬이더라.'; if (/너는\s*내일|뭐\s*할\s*예정/.test(message)) return '나는 내일도 여기서 네 질문 받으면서 도와주고 있을 것 같아. 너는 내일 뭐 하면서 쉬려고?'; return null; }
function buildWhiskyAnswer(message, state) { const topicIsWhisky = state.topic === '위스키' || /위스키|whisky|whiskey|글렌알라키|cs10|cs 10|스모키|피트|셰리|버번|싱글몰트/i.test(message); if (!topicIsWhisky) return null; if (/글렌알라키|cs10|cs 10/i.test(message) && /비슷|있어|추천/.test(message)) return '글렌알라키 CS10 좋아하면 진한 셰리감이랑 높은 도수에서 오는 농도가 취향인 쪽이네.\n\n비슷한 결로는 글렌드로낙 15 리바이벌, 아란 셰리 캐스크, 탐두 배치 스트렝스가 먼저 떠올라.'; if (/안\s*끌리|별로|아쉬/.test(message)) return '오케이, 그럼 CS10 비슷한 안전빵 말고 아예 방향을 틀어보자. 더 진하고 비싼 쪽이면 글렌드로낙 18, 아란 18, 글렌알라키 15가 좋고, 완전히 다른 매력으로 가면 스프링뱅크 15, 레드브레스트 21도 재밌어.'; if (/비싸|맛난|맛있는|스타일|다른|프리미엄|고급/.test(message)) return '그럼 CS10 기준에서 더 비싸고 맛도 확실한 쪽으로 보면 글렌드로낙 18, 아란 18/21, 스프링뱅크 15, 레드브레스트 21, 라가불린 16 쪽이 좋아. 하나만 고르면 글렌드로낙 18, 다른 스타일이면 스프링뱅크 15부터 볼 것 같아.'; return null; }
...
function getWeatherLocation(message) { for (const key of Object.keys(KOREA_CITY_COORDS)) if (message.includes(key)) return key; const match = message.match(/([가-힣]{2,10})\s*(날씨|기온|습도|미세먼지)/); const value = match?.[1]; if (!value || /^(오늘|내일|모레|현재|지금)$/.test(value)) return '서울'; return value; }
...
async function buildAnswer(message, userId) {
  ...
  if (isWeatherQuery(message)) { const route = routeFromIntent('weather', 0.99, 'fast_weather'); const analysis = { intent: 'weather', tool: 'weather', topic: '', source: 'fast_weather' }; return { answer: await getWeatherAnswer(message), results: [], route, topic: '', analysis }; }
...
app.post('/kakao-skill-webhook', async (req, res) => { const startedAt = Date.now(); const message = getUserMessage(req.body); const userId = getUserId(req.body); const callbackUrl = getCallbackUrl(req.body); if (!message) return res.json(kakaoTextResponse('메시지 입력해줘.')); if (isContinuationRequest(message)) return res.json(getContinuationResponse(userId)); try { if (callbackUrl) { setImmediate(() => sendCallback(callbackUrl, message, userId)); return res.json({ version: '2.0', useCallback: true, data: { text: '맥락 보고 바로 답하고 있어. 잠깐만 기다려줘.' } }); } rememberMessage(userId, 'user', message); const { answer, results, route, topic, analysis } = await buildAnswer(message, userId); rememberMessage(userId, 'assistant', answer, route, topic, analysis); console.log(`[kakao] ${Date.now() - startedAt}ms route=${route.intent}/${route.handler} source=${route.source || ''} search=${results.length} topic=${topic || ''} user=${userId} message="${message.slice(0, 80)}"`); return res.json(kakaoTextResponse(answer, getQuickReplies(results, route), userId)); } catch (error) { console.error('[kakao] failed:', { message: error.message, code: error.code, status: error.response?.status, elapsedMs: Date.now() - startedAt }); return res.json(kakaoTextResponse(buildTimeoutFallback(message, userId))); } });
app.use((req, res) => res.status(404).json({ ok: false, error: 'Not Found' }));
app.listen(PORT, () => { console.log(`Kakao skill webhook server listening on port ${PORT}`); console.log(`Router version: ${ROUTER_VERSION}`); console.log(`Claude model: ${CLAUDE_MODEL}`); });
