...
const ROUTER_VERSION = 'claude-haiku-4-5-2026-05-22m-restaurant-keyword-window';
...
const CLAUDE_TIMEOUT_MS = Math.max(Number(process.env.CLAUDE_TIMEOUT_MS || 6200), 4400);
const KAKAO_MAX_RESPONSE_LENGTH = Number(process.env.KAKAO_MAX_RESPONSE_LENGTH || 1000);
const KAKAO_HANDLER_TIMEOUT_MS = Math.min(Math.max(Number(process.env.KAKAO_HANDLER_TIMEOUT_MS || 6500), 2500), 9000);
...
const MIVE_FACE_IMAGE_BUFFER = loadAssetBuffer(path.join(CALENDAR_ASSET_DIR, 'mive-face.png'));
...
const MIVE_CALENDAR_THEME = {
  ...
  text: '#2B1020',
  muted: '#8A5B73',
  rowAccent: ['#F472B6', '#EC4899', '#DB2777', '#FB7185'],
...
function isCasualMealChoiceRequest(message) {
  const text = normalizeKoreanSearchText(message);
  if (LOCAL_LOCATION_PATTERN.test(text)) return false;
