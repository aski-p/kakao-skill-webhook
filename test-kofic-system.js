...
async function testKoficIntegratedSystem() {
    console.log('[MOVIE] 영화진흥위원회 API 통합 시스템 테스트 시작\n');
    ...
    console.log(`[MOVIE] 영화진흥위원회 API 키: ${koficApiKey ? '설정됨 (' + koficApiKey.substring(0, 8) + '...)' : '[ERROR] 미설정'}`);
    console.log(`🗄️ Supabase 연결: ${supabase.client ? '[SUCCESS] 성공' : '[ERROR] 실패'}`);
    console.log(`[SATELLITE] 네이버 API: ${process.env.NAVER_CLIENT_ID ? '[SUCCESS] 설정됨' : '[ERROR] 미설정'}`);
    
    if (!koficApiKey) {
        console.log('\n[WARN] Railway 환경변수에 KOFIC_API_KEY 추가 필요:');
        console.log('KOFIC_API_KEY=504ec8ff56d6c888399e9b9c1f719f03');
        console.log('\n[LOADING] 로컬 테스트는 제한적으로 진행됩니다.\n');
    }
    ...
    if (koficApiKey) {
        try {
            console.log('[SEARCH] 박스오피스 API 테스트 중...');
            ...
            console.log(`[INFO] ${currentYear}년 ${currentMonth}월 박스오피스 영화 수: ${monthlyMovies.length}개`);
            
            if (monthlyMovies.length > 0) {
                console.log('[MOVIE] 상위 5개 영화:');
                ...
                if (movieDetail) {
                    console.log(`\n[SEARCH] "${movieDetail.movieNm}" 상세 정보:`);
            ...
            console.log('[SUCCESS] 영화진흥위원회 API 테스트 성공');
            
        } catch (error) {
            console.log('[ERROR] 영화진흥위원회 API 테스트 실패:', error.message);
        }
    } else {
        console.log('[WARN] API 키 없음 - 크롤러 테스트 스킵');
    }
        ...
        try {
            ...
            if (classification.category === 'MOVIE_REVIEW') {
                ...
                if (extractionResult.success) {
                    ...
                    console.log('[SUCCESS] 영화평 생성 성공');
                    
                    // 데이터 소스 확인
                    if (message.includes('Supabase 영화 데이터베이스')) {
                        console.log('[INFO] 데이터 소스: Supabase (영화진흥위원회 + 네이버)');
                    } else if (message.includes('공개 영화 데이터베이스')) {
                        console.log('[INFO] 데이터 소스: 로컬 공개 DB');
                    } else {
                        console.log('[INFO] 데이터 소스: 네이버 API (실시간)');
                    }
                    ...
                    console.log(`정보 완성도: 감독(${hasDirector ? '[SUCCESS]' : '[ERROR]'}) 출연진(${hasCast ? '[SUCCESS]' : '[ERROR]'}) 평론가(${hasCriticReview ? '[SUCCESS]' : '[ERROR]'}) 관객(${hasAudienceReview ? '[SUCCESS]' : '[ERROR]'})`);
                } else {
                    console.log('[ERROR] 영화평 생성 실패');
                }
            }
            
        } catch (error) {
            console.log('[ERROR] 테스트 오류:', error.message);
        }
    }
    ...
    if (supabase.client) {
        try {
            ...
            console.log(`[MOVIE] 총 영화 수: ${totalMovies || 0}개`);
            ...
            console.log('[ERROR] 데이터베이스 조회 오류:', error.message);
        }
    }
    ...
    console.log('\n5️⃣ 시스템 구성 요약');
    console.log('='.repeat(50));
    console.log('[TARGET] 영화평 검색 우선순위:');
    ...
    console.log('\n[PARTY] 영화진흥위원회 API 통합 시스템 테스트 완료!');
