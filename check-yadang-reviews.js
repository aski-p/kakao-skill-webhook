...
async function checkYadangReviews() {
    ...
    try {
        console.log('[MOVIE] 야당 영화 평론가 리뷰 확인 중...');
        ...
        if (movieError) {
            console.error('[ERROR] 영화 검색 오류:', movieError);
            return;
        }
        
        if (!movies || movies.length === 0) {
            console.log('[ERROR] 야당 영화를 찾을 수 없습니다.');
            return;
        }
        
        const movie = movies[0];
        console.log(`[SUCCESS] 야당 영화 발견:`);
        ...
        if (reviewError) {
            console.error('[ERROR] 리뷰 검색 오류:', reviewError);
            return;
        }
        
        console.log(`\n[FORM] 평론가 리뷰 개수: ${reviews?.length || 0}개`);
        ...
        if (reviews && reviews.length > 0) {
            ...
            console.log('[SEARCH] 필수 평론가 확인:');
            console.log(`   박평식: ${hasParkPyeongSik ? '[SUCCESS] 있음' : '[ERROR] 없음'}`);
            console.log(`   이동진: ${hasLeeDongJin ? '[SUCCESS] 있음' : '[ERROR] 없음'}`);
            
        } else {
            console.log('[ERROR] 평론가 리뷰가 없습니다.');
            console.log('\n[TIP] 해결 방법:');
